...
/*
    ...
    NOT found             = 0
*/
function ChkProperty( auntSueProp:Array<any>, propS:string, propN:number ) : number
{
    var rV:number = 0;
...
for ( var i = 1; i <= 500; i++ )
{
    const c0 = ChkProperty( AuntSues2[i], "children"   , 3 );
    const c1 = ChkProperty( AuntSues2[i], "cats"       , 7 );
    const c2 = ChkProperty( AuntSues2[i], "samoyeds"   , 2 );
    const c3 = ChkProperty( AuntSues2[i], "pomeranians", 3 );
    const c4 = ChkProperty( AuntSues2[i], "akitas"     , 0 );
    const c5 = ChkProperty( AuntSues2[i], "vizslas"    , 0 );
    const c6 = ChkProperty( AuntSues2[i], "goldfish"   , 5 );
    const c7 = ChkProperty( AuntSues2[i], "trees"      , 3 );
    const c8 = ChkProperty( AuntSues2[i], "cars"       , 2 );
    const c9 = ChkProperty( AuntSues2[i], "perfumes"   , 1 );
